...
function renderInstructions(
  ...
  return [
    ...
    "",
    "Step 2.5 — REQUIRED if any action is `http_request` and a later step",
    "           references `{{stepN.body.PATH}}`: probe the endpoint with",
    "           curl FIRST and verify PATH exists in the actual response.",
    "           Do not infer the response shape from the URL or the user's",
    "           description — engines silently render undefined paths as",
    "           empty strings, so wrong paths produce successful runs whose",
    "           messages contain blank placeholders (e.g. `Temp: °C`).",
    "  e.g.  curl -s '<url>' | head -c 1000",
    "        # confirm body.current.temperature_2m → 11.8 (✓), not undefined",
    "  Bake the verified path into the action template before continuing.",
    "",
    ...
    "",
    "  If the plan uses `{{stepN.body.PATH}}` templating, ALSO read back the",
    "  delivered message after fire-once and confirm the placeholders rendered",
    "  with real values, not empty strings. Dry-run does NOT exercise template",
    "  substitution against live data.",
    "",
