...
function renderInstructions(
  ...
  return [
    ...
    "",
    "Step 2.5 — REQUIRED if you are changing `actions` and a later step",
    "           references `{{stepN.body.PATH}}` from an `http_request`:",
    "           curl the endpoint FIRST and verify PATH resolves in the",
    "           actual response. Do not infer response shape from the URL.",
    "           Engines silently render undefined paths as empty strings,",
    "           so wrong paths produce 'successful' runs whose messages",
    "           contain blank placeholders (e.g. `Temp: °C`).",
    ...
    "",
    "  After saving an `actions` change that uses `{{stepN.body.PATH}}`,",
    "  fire once for real and read back the delivered message — dry-run",
    "  does not exercise template substitution against live data:",
    `    ano automation run ${id} --no-dry-run --agent`,
    "    ano messages read --channel <dm-channel-id> --limit 1 --agent",
    "  Confirm placeholders rendered with real values, not empty strings.",
    "",
